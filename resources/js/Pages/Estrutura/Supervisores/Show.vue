  ...
  <app-layout title="Dashboard">
    <template #header>
      ...
      <DialogModal :show="showModal">
        <template #content>
          <list-item class="mb-8  items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest"> EDIÇÃO DE SUPERVISOR</list-item>
...
export default defineComponent({
  ...
  methods: {
    submit() {
      this.form.put(
        this.route("estrutura_supervisor.update", this.supervisor.id)
      );
      this.showModal = false;
    },
    submitDelete() {
      this.formDelete.delete(
        this.route("estrutura_supervisor.destroy", this.supervisor.id)
      );
